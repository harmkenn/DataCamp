...
NBAplayers <- read.csv("../datasets/NBAplayers.csv")
 x <- NBAplayers %>% select(X3P) %>% filter(X3P > 0) %>% pull() %>% na.omit() %>% as.numeric()
...
server <- function(input, output) {
 ...
 output$distPlot <- renderPlot({
    ...
    x <- NBAplayers %>% select(input$stat) %>% filter(input$stat >= 0) %>% pull() %>% na.omit() %>% as.numeric() # select columns of NBA
